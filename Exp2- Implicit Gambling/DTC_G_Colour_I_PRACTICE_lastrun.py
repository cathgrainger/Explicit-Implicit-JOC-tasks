...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.83.04), Fri 12 Oct 11:20:02 2018
...
expName = 'Gambling Task- Line version (2)'  # from the Builder filename that created this script
...
filename = _thisDir + os.sep + 'data/%s_%s_%s' %(expInfo['participant'], "Colour Implicit", expInfo['date'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath=u'/Users/tobynicholson/Dropbox/2018.student projects/Group 2 - Ruth and Jaime - Gambling/Dual task/Gambling/DTC_G_Colour_I_PRACTICE.psyexp',
...
text_2 = visual.TextStim(win=win, ori=0, name='text_2',
    ...
    pos=[0.90, -0.9], height=0.05, wrapWidth=None,
...
polygon_7 = visual.ShapeStim(win=win, name='polygon_7',
    ...
    opacity=1,depth=0.0, 
interpolate=True)
polygon_8 = visual.Polygon(win=win, name='polygon_8',
...
polygon = visual.ShapeStim(win=win, name='polygon',
    ...
    opacity=1,depth=0.0, 
interpolate=True)
polygon_9 = visual.Polygon(win=win, name='polygon_9',
    edges = 100, size=[0.25, 0.25],
    ori=0, pos=[0.3, 0],
    lineWidth=1, lineColor=[-1,-1,-1], lineColorSpace='rgb',
    fillColor=[1,1,1], fillColorSpace='rgb',
...
for thisTrial in trials:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_2 = data.TrialHandler(nReps=runorskip, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
...
Instructions_2Components.append(text_4)
...
for thisTrial_3 in trials_3:
    ...
    sound_aloneComponents.append(text_9)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *text_9* updates
        if t >= 6 and text_9.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_9.tStart = t  # underestimates by a little under one frame
            text_9.frameNStart = frameN  # exact frame index
            text_9.setAutoDraw(True)
        if text_9.status == STARTED and t >= (6 + (2-win.monitorFramePeriod*0.75)): #most of one frame period left
...
instructions_3Components.append(text_10)
...
for thisTrial_5 in trials_5:
    ...
    # keep track of which components have finished
    trial2Components = []
    trial2Components.append(text_11)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if text_11.status == STARTED and t >= (1 + (3-win.monitorFramePeriod*0.75)): #most of one frame period left
        ...
        if text_12.status == STARTED:  # only update if being drawn
            text_12.setText(t3, log=False)
    ...
    text_13.setColor(feedback_colour, colorSpace='rgb')
    text_13.setText(score_feedback)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if text_13.status == STARTED and t >= (0.0 + (1-win.monitorFramePeriod*0.75)): #most of one frame period left
...
# keep track of which components have finished
bridgeComponents = []
bridgeComponents.append(text_14)
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if text_14.status == STARTED and t >= (0.0 + (2-win.monitorFramePeriod*0.75)): #most of one frame period left
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if text_15.status == STARTED and t >= (0.0 + (2-win.monitorFramePeriod*0.75)): #most of one frame period left
